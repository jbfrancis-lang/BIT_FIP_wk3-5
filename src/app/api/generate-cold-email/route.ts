...
export async function POST(request: Request) {
  try {
    ...
    const generated = await generateJson<ColdEmailOutput>({
      system:
        "당신은 대학 학회와 학생 조직을 위한 공식 대외협력 제안 메일 작성자입니다. 학생 조직의 진정성은 유지하되, 전체 문체는 정중하고 공적인 B2B/산학협력 제안 톤이어야 합니다. 과도하게 캐주얼하거나 친근한 표현을 피하고, 검토 요청, 협업 가능성 논의, 담당 부서 연결 요청, 회신 요청 같은 공식 표현을 사용하세요. 제목 3개, 메시지 전략, 전환 전략, 이메일 본문, 짧은 DM, 후속 메일, 한 문장 피치, CTA 문장, 품질 체크리스트가 모두 같은 공식 문체를 유지해야 합니다. 실제 B2B 콜드메일과 파트너십 제안 메일의 일반 원칙을 반영해 수신 기업 중심의 개인화, 문제 또는 기회 중심 도입, 명확한 기대가치, 하나의 구체적인 다음 행동 요청을 우선하세요. 이메일 본문에는 기업 선정 이유, 예상 문제 또는 기회, 협업 방향, 기업 기대가치, 명확한 미팅 또는 회신 요청을 포함하세요. 대상 기업의 문제 상황, 해소 가능 영역, 왜 이 학회가 제안하는지가 자연스럽게 드러나야 합니다. 공개되지 않은 개인 연락처나 사적 정보를 만들지 마세요. LinkedIn은 직접 스크래핑하거나 자동 DM을 전제로 하지 말고, 사용자가 직접 확인한 담당자/관계 힌트가 있을 때만 문장에 반영하세요.",
      user: {
        ...
        output_guidelines: {
          ...
          conversionStrategy: "Who와 How 관점의 전환 전략을 3~4문장으로 작성. 추천 수신자 유형, 수동 확인한 관계/활동성 힌트, 제목/첫 문장 hook, 첫 CTA와 후속 액션을 포함",
        ...
        quality_rules: [
          ...
          "수신자 이름, 직책, 동문 여부, LinkedIn 활동성은 사용자가 입력한 경우에만 반영하세요.",
          "LinkedIn API, 스크래핑, 자동 1촌 신청, 자동 DM을 전제로 하는 표현은 작성하지 마세요.",
    ...
    const conversionNotes = buildConversionStrategyNotes(payload);
    const output = {
      ...generated,
      conversionStrategy: generated.conversionStrategy || conversionNotes.summary,
      ...linkedInTemplates
    };
